...
from ai_assistant.modules.llm_provider import LLMFactory
    ...
    def validate_plan(plan: TaskPlan) -> tuple[bool, str]:
        ...
        for domain in PlanValidator.BLACKLISTED_DOMAINS:
            if domain in command_lower:
                return False, f"❌ Operation blocked: contains blacklisted term '{domain}'"
        
        # Check action count (prevent infinite loops)
        if len(plan.actions) > 50:
            return False, "❌ Too many actions (>50). Please simplify the command."
        
        # Check for circular dependencies
        if PlanValidator._has_circular_dependencies(plan.actions):
            return False, "❌ Circular dependencies detected in action plan"
