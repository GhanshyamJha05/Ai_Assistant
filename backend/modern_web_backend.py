...
try:
    ...
    print("Ã¢Å“â€¦ Automation tools loaded successfully")
except ImportError as e:
    ...
    try:
        ...
        AUTOMATION_AVAILABLE = True
        print("Ã¢Å“â€¦ App discovery loaded from modules")
...
def _get_learning_router_lazy():
    ...
    if learning_router is None and LEARNING_ROUTER_AVAILABLE:
        try:
            ...
            logger.info("Ã¢Å“â€¦ Learning router initialized - AI will learn from all interactions")
...
def _get_memory_retriever_lazy():
    ...
    if memory_retriever is None and SMART_MEMORY_AVAILABLE:
        try:
            ...
            logger.info("Ã¢Å“â€¦ Smart memory retrieval initialized - AI can answer from past conversations")
        except Exception as e:
...
def _get_enhanced_ai_lazy():
    ...
    if enhanced_ai is None and ENHANCED_AI_AVAILABLE:
        try:
            ...
            logger.info("Ã¢Å“â€¦ Enhanced AI initialized (semantic cache, routing, streaming, emotion, verification)")
        except Exception as e:
...
def _get_usage_analyzer_lazy():
    ...
    if usage_analyzer is None and USAGE_ANALYZER_AVAILABLE:
        try:
            ...
            logger.info("Ã¢Å“â€¦ Usage pattern analyzer initialized")
        except Exception as e:
...
try:
    from learning_dashboard_api import dashboard_api
...
logger.info("Ã¢Å“â€¦ SocketIO initialized with CORS origins: %s", ALLOWED_ORIGINS)
...
# Ã¢Å¡Â¡ OPTIMIZATION: Lazy load Ollama - only initialize when user selects it
...
logger.info("Ã¢Å¡Â¡ Ollama will load on-demand (lazy loading enabled)")
...
try:
    ...
    logger.info("Ã¢Å“â€¦ Voice API blueprint registered at /api/voice")
    ...
    if voice_initialized:
        ...
        logger.info("=" * 60)
        logger.info("Ã¢Å“â€¦ SmartWakeWordDetector - PocketSphinx (Offline)")
        logger.info("Ã¢Å“â€¦ NeuralVoiceEngine - Edge-TTS + Coqui")
        logger.info("Ã¢Å“â€¦ VoiceActivityDetector - WebRTC VAD")
        logger.info("Ã¢Å“â€¦ Speaker Recognition - Enabled")
        logger.info("Ã¢Å“â€¦ Advanced STT - Whisper + Google + Vosk")
        logger.info("Ã¢Å“â€¦ Noise Reduction - Active")
        logger.info("=" * 60)
    else:
...
if not _admin_password:
    logger.warning(
        "Ã¢Å¡Â Ã¯Â¸Â  ADMIN_PASSWORD not set! Using temporary generated password. "
...
try:
    print("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â§ Initializing YourDaddy Assistant...")
    assistant = ModernAssistant()
    print("ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Assistant initialized successfully")
except Exception as e:
    print(f"ÃƒÂ¢Ã‚ÂÃ…â€™ CRITICAL: Assistant initialization failed: {e}")
    print("ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â  Server will start in limited mode without some features")
...
def test_page():
    ...
    <body>
        <div class="container">
            <h1>ÃƒÂ°Ã…Â¸Ã‚Â¤Ã¢â‚¬â€œ YourDaddy Assistant - Test Page</h1>
            <div class="status success">ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Backend is operational and all features are enabled!</div>
            
            <h2>Test Enhanced Chat Features:</h2>
            <a href="/enhanced-chat" class="test-btn">ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ Open Enhanced Chat Interface</a>
            <a href="/api/features" class="test-btn">ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â§ Check Available Features</a>
            <a href="/api/apps" class="test-btn">ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â± List Installed Applications</a>
            <a href="/api/weather" class="test-btn">ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¤ÃƒÂ¯Ã‚Â¸Ã‚Â Get Weather Information</a>
...
def api_chat_stream():
    ...
    try:
        ...
        logger.info(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Å¾ Streaming chat for user: {current_user}, session: {session_id}")
        ...
        def generate_stream():
            ...
            try:
                ...
                logger.info(f"ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Stream complete: {tokens} tokens in {duration:.2f}s ({tokens/duration:.1f} tok/s)")
...
def handle_chat_stream(data):
    ...
    try:
        ...
        logger.info(f"ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ WebSocket stream complete: {tokens} tokens in {duration:.2f}s")
...
def handle_voice_command(data):
    ...
    try:
        ...
        print(f"Ã¢Å“â€¦ Voice command processed successfully")
        print(f"   Response: {response[:100]}...")
        print(f"Ã°Å¸â€Å  Emitted voice_response event for talkback")
        
    except Exception as e:
        import traceback
        error_trace = traceback.format_exc()
        print(f"Ã¢ÂÅ’ Voice command error: {str(e)}")
    ...
    try:
        ...
        if category == 'ai':
            ...
            if provider or model:
                logger.info(f"Ã°Å¸Â¤â€“ AI provider updated: {provider}, model: {model}")
...
if VOICE_API_AVAILABLE and 'voice_bp' in globals():
    try:
        ...
        if not any(bp.name == 'voice' for bp in app.blueprints.values()):
            ...
            logger.info("Ã¢Å“â€¦ Voice API blueprint already registered (skipping duplicate)")
    except Exception as e:
...
try:
    ...
    logger.info("Ã¢Å“â€¦ Chat & Voice Socket.IO handlers registered")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ Security: JWT authentication enabled")
        print(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ Security: Rate limiting enabled")
        print(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ Security: CORS restricted to: {', '.join(ALLOWED_ORIGINS)}")
        print(f"ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ Security: Host binding: {host}")
        print("")
        print(f"ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â  Default credentials: username='admin', password='{os.getenv('ADMIN_PASSWORD', 'changeme123')}'")
        print("ÃƒÂ¢Ã…Â¡Ã‚Â ÃƒÂ¯Ã‚Â¸Ã‚Â  CHANGE THE PASSWORD in .env file before production!")
        print("")
        ...
        try:
            ...
            print("Ã¢Å“â€¦ System monitoring started")
        except ImportError as e:
        ...
        if GOOGLE_SPEECH_WS_AVAILABLE:
            try:
                ...
                print("Ã¢Å“â€¦ Google Speech Recognition WebSocket handlers registered")
            except Exception as e:
        ...
        try:
            ...
            print("Ã¢Å“â€¦ Command handlers registered with local-first routing")
        except Exception as e:
        ...
        print(f"ÃƒÂ¢Ã‚ÂÃ…â€™ Server failed to start: {e}")
        sys.exit(1)
