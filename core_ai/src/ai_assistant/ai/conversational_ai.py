...
class AdvancedConversationalAI:
    ...
    def _execute_open_command(self, query: str, query_lower: str) -> str:
        ...
        # ðŸ”¥ CRITICAL FIX: Use Intent Recognizer to normalize app name
        ...
        if any(indicator in app_name for indicator in ['.com', '.org', '.net', '.io', '.app', '.dev', 'http', 'www.']):
            ...
            try:
                webbrowser.open(url)
                return self._format_success(f"âœ… Opening {url} in your browser", f"open {url}")

            except Exception as e:
                return f"âŒ Could not open website: {str(e)}"
        ...
        if app_name in app_mappings:
            ...
            if target.startswith('http'):
                try:
                    ...
                    return self._format_success(f"âœ… Opening {app_name.title()}", f"open {app_name}")

                except Exception as e:
                    return f"âŒ Could not open: {str(e)}"
            
            # Otherwise it's an executable
            try:
                subprocess.Popen(target, shell=True)
                success_msg = self._format_success(f"âœ… Opening {app_name.title()}", f"open {app_name}")
        ...
        try:
            ...
            return f"âŒ Could not find application '{app_name}'. Try being more specific or check if it's installed."
    ...
    def _execute_close_command(self, query: str, query_lower: str) -> str:
        ...
        try:
            ...
            return f"âŒ Could not close '{app_name}'. It may not be running."
        except Exception as e:
            return f"âŒ Error closing application: {str(e)}"
    ...
    def _execute_search_command(self, query: str, query_lower: str) -> str:
        ...
        # ðŸŒŸ CONTEXT AWARENESS: Check Active Window
        ...
        try:
            ...
            return f"âŒ Search failed: {str(e)}"
    ...
    def _execute_play_command(self, query: str, query_lower: str) -> str:
        ...
        try:
            ...
            return f"âŒ Could not play: {str(e)}"
    ...
    def _execute_create_document(self, query: str, query_lower: str) -> str:
        ...
        if 'ppt' in query_lower or 'powerpoint' in query_lower or 'presentation' in query_lower:
            try:
                ...
                return "âŒ PowerPoint not found. Please install Microsoft Office."
            ...
            try:
                ...
                return "âŒ Word not found. Please install Microsoft Office."
